...
<div class="card p-4">
    <div class="d-flex flex-column flex-md-row gap-3 align-items-md-center mb-4">
        ...
        <div>
            ...
            <p class="text-muted mb-2">{{ $employee->employee_id }} - {{ ucfirst($employee->job_role) }} - {{ ucfirst($employee->user->role) }}</p>
    ...
    <div class="row g-3">
        ...
        <div class="col-md-4"><div class="border rounded p-3">Telepon<br><strong>{{ $employee->phone ?? '-' }}</strong></div></div>
        <div class="col-md-4"><div class="border rounded p-3">Alamat<br><strong>{{ $employee->address ?? '-' }}</strong></div></div>
        <div class="col-md-3"><div class="border rounded p-3">Jam Kerja<br><strong>08:00 - 17:00</strong></div></div>
        <div class="col-md-3"><div class="border rounded p-3">RFID<br><strong>{{ $employee->rfidCards->first()->uid ?? '-' }}</strong></div></div>
        <div class="col-md-3"><div class="border rounded p-3">Gaji<br><strong>Rp {{ number_format($employee->salary,0,',','.') }}</strong></div></div>
    ...
    <div class="mt-4 d-flex gap-2">
        ...
        <form id="delete-employee-detail-{{ $employee->id }}" method="POST" action="{{ route('admin.employees.destroy', $employee) }}">
            @csrf
            @method('DELETE')
            <button
